...
module Puptime
  ...
  class CLI
    ...
    private
      ...
      def setup_logging
        log_file = File.expand_path(@options[:log_file] || Puptime::Constant::LOG_FILE_PATH).freeze
        Puptime::Logging.setup_logger(log_file)
      end

      def set_pid
        pid_file = File.expand_path(@options[:pid_file] || Puptime::Constant::PID_FILE_PATH).freeze
